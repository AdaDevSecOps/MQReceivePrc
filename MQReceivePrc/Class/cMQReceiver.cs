...
namespace MQReceivePrc.Class
{
    public class cMQReceiver
    {
        ...
        private List<IConnection> aoC_MQConn;  //*Net 63-10-22 ใช้เป็นหลาย Connection หลาย Channel เหมือนเดิม
        ...
        public void C_MQRxProcess()
        {
            ...
            try
            {
                ...
                if (oC_Config.C_CFGbLoadConfig(out tMsgErr))
                {
                    ...
                    //*Ton 2021-10-18 Conntectoin Testing *********
                    Console.WriteLine($"DbServer:{oC_Config.oC_ShopDB.tServer}");
                    Console.WriteLine($"DbName  :{oC_Config.oC_ShopDB.tDatabase}");
                    Console.WriteLine($"DbUser  :{oC_Config.oC_ShopDB.tUser}");

                    // Test Connect Database
                    cDatabase oDB = new cDatabase();
                    string tTestConnection = oDB.C_DAToExecuteQuery<string>(cVB.tVB_ConnStr, "SELECT DB_NAME() AS DatabaseName",30);
                    string tStaConnDB = null;
                    if (string.IsNullOrEmpty(tTestConnection))
                    {
                        tStaConnDB = "Connect Database false!";
                    }
                    else
                    {
                        tStaConnDB = "Connect Database success.";
                    }
                    Console.WriteLine($"Status  :{tStaConnDB}");

                    Console.WriteLine($"++++++++++ RabbitMQ Connection Info ++++++++++");
                    Console.WriteLine($"tMQHostName   :{oC_Config.oC_RabbitMQ.tMQHostName}");
                    Console.WriteLine($"tMQUserName   :{oC_Config.oC_RabbitMQ.tMQUserName}");
                    Console.WriteLine($"tMQVirtualHost:{oC_Config.oC_RabbitMQ.tMQVirtualHost}");
                    Console.WriteLine($"RabbitMQ EndPoint:{oFactory.Endpoint}");
                    ...
                    //*Net 63-09-02 แสดงชื่อ Console //*Net 63-10-02 Comment
                    ...
                    //oC_MQConn = oFactory.CreateConnection(); //*Net 63-09-02 สร้าง 1 Connection พอ //*Net 63-10-27
                    aoC_MQConn = new List<IConnection>(); //*Net 63-10-27 สร้าง List
                    ...
                    foreach (string tQueue in atQueue)
                    {
                        ...
                        //oChannel = oC_MQConn.CreateModel(); //*Net 63-09-02 //*Net 63-10-27
                        ...
                        aoC_MQConn.Add(oConn); //*Net 63-10-27 เก็บไว้ใน List

                    }

                    //*Net 63-10-27 ปรับการ check Connection
                ...
                else
                {
                    ...
                    //Console.ReadLine();
                    Task.Delay(10000).Wait(); //*Net 63-10-27
                }
            }
                ...
                while (bStaConsume == false)
                {
                    try
                    {
                        ...
                        oConsumer.Received += (oModel, oEevntArgs) =>
                        {
                            ...
                            aoBody = oEevntArgs.Body.ToArray();//*Ton 2021-09-09
                            ...
                            switch (ptQueue)
                            {
                                ...
                                case "CN_QTask": //*Arm 65-08-22 Queue สำหรับ AdaTask
                                    try
                                    {
                                        Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "]" + " Center Q Task Process Start...");
                                        cmlRcvData oRcvData = Newtonsoft.Json.JsonConvert.DeserializeObject<cmlRcvData>(tMessage);
                                        cCNTask oCNTask = new cCNTask();
                                        bPrc = oCNTask.C_PRCbAdaTask(tQueueID, oRcvData, oC_Config.oC_ShopDB, ref tErrMsg);
                                        if (bPrc)
                                        {
                                            poChannel.BasicAck(oEevntArgs.DeliveryTag, false);
                                            Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "]" + " Process Center Q Task Success.");
                                        }
                                        else
                                        {
                                            if (!string.IsNullOrEmpty(tErrMsg)) Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "] " + tErrMsg);
                                            Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "]" + " Process Center Q Task false.");
                                        }
                                    ...
                                    {
                                        // ต้องเขียน log ลง text file ด้วย
                                        Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "]" + " Process CN_QTask Error " + oEx.Message.ToString());
                                        cFunction.C_LOGxKeepLogErr(oEx.Message.ToString(), "C_PRCxMessage/CN_QTask");
                                    }

                                    break;

                                case "UPLOADPDTPICK":  //*Arm 65-09-28 [CR-WMS] Upload ข้อมูลใบจัดสินค้า
                                    try
                                    {
                                        Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "]" + " Upload product pick Process Start...");
                                        cmlRcvDataUpload oDataUpld = Newtonsoft.Json.JsonConvert.DeserializeObject<cmlRcvDataUpload>(tMessage);
                                        cPdtPick oPdtPick = new cPdtPick();
                                        bPrc = oPdtPick.C_PRCbUploadPdtPick(tQueueID, oDataUpld, oC_Config.oC_ShopDB, ref tErrMsg);
                                        if (bPrc)
                                        {
                                            poChannel.BasicAck(oEevntArgs.DeliveryTag, false);
                                            Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "]" + " Upload product pick Process Success.");
                                        }
                                        else
                                        {
                                            if (C_CHKbSQlServErr(tErrMsg))
                                            ...
                                            Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "]" + " Upload product pick Process false.");
                                        }
                                    }
                                    catch (Exception oEx)
                                    {
                                        // ต้องเขียน log ลง text file ด้วย
                                        Console.WriteLine(DateTime.Now.ToString(tFmtDateTime) + " [" + tQueueID + "]" + " Upload product pick Error. " + oEx.Message.ToString());
                                        cFunction.C_LOGxKeepLogErr(oEx.Message.ToString(), "C_PRCxMessage/UPLOADPDTPICK");
                                    }
        ...
        private bool C_PRCbSalePos(string ptComName, string ptMessage)
        {
            ...
            string tStoreProcedure = ""; //*Arm 63-10-26
            try
            {
                ...
                oSql.AppendLine(", FNXshDocType"); //*Arm 63-10-26
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            //*Arm 63-10-26
                            if(odtTmp.Rows[0]["FNXshDocType"].ToString() == "9")
                            {
                                tStoreProcedure = "STP_DOCxRefundPrcStk";
                            }
                            else
                            {
                                tStoreProcedure = "STP_DOCxSalePrcStk";
                            }
                            //++++++++++++++

                            //if (oDB.C_DATbExecuteStoreProcedure(tConnStr, "STP_DOCxSalePrcStk", ref aPara,(int)oC_Config.oC_ShopDB.nCommandTimeOut, "@FNResult")) //*Arm 63-10-26 Comment Code
                            if (oDB.C_DATbExecuteStoreProcedure(tConnStr, tStoreProcedure, ref aPara, (int)oC_Config.oC_ShopDB.nCommandTimeOut, "@FNResult")) //*Arm 63-10-26
                            {
                                oSql = new StringBuilder();
