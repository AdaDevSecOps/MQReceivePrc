...
namespace MQReceivePrc.Class
{
    public class cSale
    {
        ...
        public bool C_PRCbUploadSale(string ptQueueID, cmlRcvDataUpload poData, cmlShopDB poShopDB, ref string ptErrMsg)
        {
            ...
            cDataReader<cmlTLKTSalHDCstAddr_L> aoHDCstAddr;   //*Arm 64-04-09 //*Arm 64-04-09 [CR-CH4]
            cDataReader<cmlTPSTSalHDDocRef> aoHDDocRef;     //*Arm 65-09-28 -[CR-WMS]
            ...
            try
            {
                ...
                //*Arm 64-12-09 
                oSql.Clear();
                oSql.AppendLine("SELECT FTXshDocNo FROM TPSTSalHD WITH(NOLOCK) ");
                oSql.AppendLine("WHERE FTBchCode = '" + oSalePos.aoTPSTSalHD[0].FTBchCode + "' AND FTXshDocNo = '"+ oSalePos.aoTPSTSalHD[0].FTXshDocNo + "' ");
                tDocNo = oDB.C_DAToExecuteQuery<string>(poData.ptConnStr, oSql.ToString(), (int)poShopDB.nCommandTimeOut);
                //+++++++++++++
                ...
                string tTblSalHDCstAddr = "TSHDCstAddr" + tQueueID; //*Arm 64-04-09 [CR-CH4]
                string tTblSalHDDocRef = "TSHDDocRef" + tQueueID;  //*Arm 65-09-28 -[CR-WMS]
                //+++++++++++// Net 63-08-03 ยกมาจาก Moshi
                ...
                oDB.C_PRCxCreateDatabaseTmp("TLKTSalHDCstAddr_L", tTblSalHDCstAddr, poData.ptConnStr, (int)poShopDB.nCommandTimeOut); //*Arm 64-04-09 [CR-CH4]
                oDB.C_PRCxCreateDatabaseTmp("TPSTSalHDDocRef", tTblSalHDDocRef, poData.ptConnStr, (int)poShopDB.nCommandTimeOut);  //*Arm 65-09-28 -[CR-WMS]
                #endregion
                ...
                //*Arm 64-04-09 [CR-CH4]
                if (oSalePos.aoTLKTSalHDCstAddr_L != null)
                {
                    aoHDCstAddr = new cDataReader<cmlTLKTSalHDCstAddr_L>(oSalePos.aoTLKTSalHDCstAddr_L);

                    using (SqlBulkCopy oBulkCopy = new SqlBulkCopy(oConn, SqlBulkCopyOptions.Default, oTranscation))
                    {
                        foreach (string tColName in aoHDCstAddr.ColumnNames)
                        {
                            oBulkCopy.ColumnMappings.Add(tColName, tColName);
                        }

                        oBulkCopy.BatchSize = 100;
                        oBulkCopy.DestinationTableName = "dbo." + tTblSalHDCstAddr;

                        try
                        {
                            oBulkCopy.WriteToServer(aoHDCstAddr);
                        }
                        catch (Exception oEx)
                        {
                            oTranscation.Rollback();
                            ptErrMsg = oEx.Message.ToString();
                            cFunction.C_LOGxKeepLogErr("BulkCopy/" + tTblSalHDCstAddr + " " + oSalePos.aoTPSTSalHD[0].FTXshDocNo + " : " + oEx.Message.ToString(), "C_PRCbUploadSale");
                            return false;
                        }
                    }
                }
                ...
                //*Arm 65-09-28 -[CR-WMS]
                if (oSalePos.aoTPSTSalHDDocRef != null)
                {
                    aoHDDocRef = new cDataReader<cmlTPSTSalHDDocRef>(oSalePos.aoTPSTSalHDDocRef);
                    ...
                    using (SqlBulkCopy oBulkCopy = new SqlBulkCopy(oConn, SqlBulkCopyOptions.Default, oTranscation))
                    {
                        ...
                        oBulkCopy.BatchSize = 100;
                        oBulkCopy.DestinationTableName = "dbo." + tTblSalHDDocRef;
                        ...
                        catch (Exception oEx)
                        {
                            oTranscation.Rollback();
                            ptErrMsg = oEx.Message.ToString();
                            cFunction.C_LOGxKeepLogErr("BulkCopy/" + tTblSalHDDocRef + " " + oSalePos.aoTPSTSalHD[0].FTXshDocNo + " : " + oEx.Message.ToString(), "C_PRCbUploadSale"); //*Arm 63-05-30
                            return false;
                        }
                    }
                }
                ...
                oSql.AppendLine("   THDTMP.FTXshStaRefund = THD.FTXshStaRefund,");  //*Arm 63-09-16 
                oSql.AppendLine("   THDTMP.FNXshStaRef = THD.FNXshStaRef,");        //*Arm 63-09-16
                ...
                oSql.AppendLine("   TDTTMP.FCXsdQtyRfn = TDT.FCXsdQtyRfn,");        //*Arm 63-09-16 
                oSql.AppendLine("   TDTTMP.FCXsdQtyLef = TDT.FCXsdQtyLef,");        //*Arm 63-09-16
                ...
                oSql.AppendLine("   AND TDTTMP.FNXsdSeqNo = TDT.FNXsdSeqNo"); //*Arm 63-09-16 
                //--------------------------------------------
                ...
                //*Arm 64-04-09 [CR-CH4]
                oSql.AppendLine("   DELETE FROM TLKTSalHDCstAddr_L WITH(ROWLOCK)");
                oSql.AppendLine("   WHERE FTBchCode = '" + oSalePos.aoTPSTSalHD[0].FTBchCode + "' AND FTXshDocNo = '" + oSalePos.aoTPSTSalHD[0].FTXshDocNo + "'");
                oSql.AppendLine();
                oSql.AppendLine("   INSERT INTO TLKTSalHDCstAddr_L");
                oSql.AppendLine("   SELECT * FROM " + tTblSalHDCstAddr + " WITH(NOLOCK) ");
                oSql.AppendLine();
                //+++++++++++++++

                //*Arm 65-09-28 -[CR-WMS]
                oSql.AppendLine("   DELETE FROM TPSTSalHDDocRef WITH(ROWLOCK)");
                oSql.AppendLine("   WHERE FTBchCode = '" + oSalePos.aoTPSTSalHD[0].FTBchCode + "' AND FTXshDocNo = '" + oSalePos.aoTPSTSalHD[0].FTXshDocNo + "'");
                oSql.AppendLine();
                oSql.AppendLine("   INSERT INTO TPSTSalHDDocRef");
                oSql.AppendLine("   SELECT * FROM " + tTblSalHDDocRef + " WITH(NOLOCK) ");
                oSql.AppendLine();
                //++++++++++++++
                ...
                if (cVB.bVB_StaUseCentralized == true) // ใช้งานระบบ Centralized
                {
                    foreach (cmlTPSTSalHD oHD in oSalePos.aoTPSTSalHD)
                    {
                        ...
                        if (oHD.FNXshDocType == 9) //*Arm 63-10-26 กรณีบิลคืนให้ส่ง SalePos
                        {
                            string tStaPrcStk = "";
                        ...
                        oSql.Clear();
                        oSql.AppendLine("SELECT ISNULL(FTPosStaPrnEJ,'') AS FTPosStaPrnEJ FROM TCNMPos WITH(NOLOCK) WHERE FTBchCode = '" + oHD.FTBchCode.ToString() + "'");
                        string tStaPrnEJ = oDB.C_DAToExecuteQuery<string>(poData.ptConnStr, oSql.ToString(), (int)poShopDB.nCommandTimeOut);
                ...
                oSql.AppendLine("DROP TABLE " + tTblTxnSale + ""); 
                oSql.AppendLine("DROP TABLE " + tTblTxnRedeem + "");
                oSql.AppendLine("DROP TABLE " + tTblSalHDCstAddr + ""); //*Arm 64-04-09 [CR-CH4]
                oSql.AppendLine("DROP TABLE " + tTblSalHDDocRef + "");  //*Arm 65-09-28 [CR-WMS]
                oDB.C_DATbExecuteNonQuery(poData.ptConnStr, oSql.ToString(), (int)poShopDB.nCommandTimeOut, out nRowAffect);
                //+++++++++++++


                if (string.IsNullOrEmpty(tDocNo)) //*Arm 64-12-09
                {
                    ...
                    if (cVB.bVB_StaAlwSendAdaLink == true) //*Arm 63-08-04
                    {
                        foreach (cmlTPSTSalHD oData in oSalePos.aoTPSTSalHD)
                        {
                            ...
                            //*Arm 65-11-10
                            oSql.Clear();
                            oSql.AppendLine("SELECT TOP 1 FTAgnCode FROM TCNMBranch WITH(NOLOCK) WHERE FTBchCode = '" + oData.FTBchCode + "' ");
                            string tQueueName = "LK_QSale2Vender" + oDB.C_DAToExecuteQuery<string>(poData.ptConnStr, oSql.ToString(), (int)poShopDB.nCommandTimeOut);
                            cFunction.C_PRCxMQPublish(tQueueName, tMsgExpSale, out ptErrMsg);
                            //++++++++++++++
                        }
                    }
                    //++++++++++++++
            ...
            finally
            {
                ...
                oDB = null;
                oConn = null;
                aoHDCstAddr = null; //*Arm 64-04-09 [CR-CH4]
                aoHDDocRef = null; //*Arm 65-09-28 [CR-WMS]
