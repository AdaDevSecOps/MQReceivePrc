...
namespace MQReceivePrc.Class
{
    public class cEJ
    {
		...
		//Net 65-09-06
		private string tC_IsoCodeCty;
		private string tC_IsoCodeLocal;
		private string tC_IsoNameLocal;
		private string tC_IsoNameCty;
		private string tC_AgnCode;
		private string tC_CtyCode;
		private decimal cC_RteCty;
		private decimal cC_RteLocal;
		private string tC_FmtPntSlip;
		//++++++++++++++++++++++++++
		private cSP oC_SP;
        ...
        private void C_GETxConfig()
        {
            ...
            try
            {
                ...
                tC_FmtPntSlip = "003"; //Net 65-09-10
				///////////////////////////////////
				#endregion
		...
		private void C_GETxConfigBranch(string ptBchCode) //Net 65-09-06
		{
			StringBuilder oSql;
			cDatabase oDB;
			...
			try
			{
				...
				//หาประเทส
				oSql.Clear();
				oSql.AppendLine($"SELECT TOP 1 ISNULL(BCH.FTPplCode, AGN.FTPplCode) AS FTPplCode ");
				oSql.AppendLine($",BCH.FTBchRefID,BCH.FTWahCode,BCH.FTAgnCode,WAH.FTWahStaChkStk, WAH.FTWahStaPrcStk");
				oSql.AppendLine($", CASE WHEN ISNULL(BCH.FTCtyCode,'')<>'' THEN ISNULL(BCH.FTCtyCode,'') ");
				oSql.AppendLine($"       ELSE CASE WHEN ISNULL(AGN.FTCtyCode,'')<>'' THEN ISNULL(AGN.FTCtyCode,'') ");
				oSql.AppendLine($"                 ELSE (SELECT TOP 1 ISNULL(FTCtyCode,'') FROM TCNMComp) ");
				oSql.AppendLine($"            END ");
				oSql.AppendLine($"  END AS FTCtyCode ");
				oSql.AppendLine($", CASE WHEN ISNULL(BCH.FTCtyCode,'')<>'' THEN (SELECT TOP 1 ISNULL(FTRteIsoCode,'') FROM TCNMCountry WHERE FTCtyCode = BCH.FTCtyCode) ");
				oSql.AppendLine($"       ELSE CASE WHEN ISNULL(AGN.FTCtyCode,'')<>'' THEN (SELECT TOP 1 ISNULL(FTRteIsoCode,'') FROM TCNMCountry WHERE FTCtyCode = AGN.FTCtyCode) ");
				oSql.AppendLine($"                 ELSE (SELECT TOP 1 ISNULL(CTY.FTRteIsoCode,'') FROM TCNMComp CMP INNER JOIN TCNMCountry CTY ON CMP.FTCtyCode=CTY.FTCtyCode) ");
				oSql.AppendLine($"            END ");
				oSql.AppendLine($"  END AS FTRteIsoCode ");
				oSql.AppendLine($"FROM TCNMBranch BCH WITH(NOLOCK)");
				oSql.AppendLine($"LEFT JOIN TCNMWaHouse WAH WITH(NOLOCK) ON BCH.FTBchCode=WAH.FTBchCode AND BCH.FTWahCode = WAH.FTWahCode");
				...
				oDbTblResult = oDB.C_DAToExecuteQuery(tC_tConnStr, oSql.ToString(), cVB.nVB_CmdTime);
                if (oDbTblResult == null || oDbTblResult.Rows.Count == 0) return;

				tC_AgnCode = oDbTblResult.Rows[0]["FTAgnCode"].ToString();
				tC_CtyCode = oDbTblResult.Rows[0]["FTCtyCode"].ToString();
                tC_IsoCodeCty = oDbTblResult.Rows[0]["FTRteIsoCode"].ToString();
                tC_IsoNameCty = cRate.C_GETtIsoName(tC_tConnStr, tC_AgnCode, tC_IsoCodeCty);

				oSql.Clear();
				oSql.AppendLine($"SELECT TOP 1 ISNULL(FCRteRate,0) AS FCRteRate ");
				oSql.AppendLine($"FROM TFNMRate ");
				oSql.AppendLine($"WHERE FTRteIsoCode = '{tC_IsoCodeCty}' ");
				oSql.AppendLine($"    AND ( ISNULL(FTAgnCode,'')='' OR ISNULL(FTAgnCode,'')='{tC_AgnCode}' ) ");
				oSql.AppendLine($"ORDER BY ISNULL(FTAgnCode,'') DESC ");
				cC_RteCty = oDB.C_DAToExecuteQuery<decimal>(tC_tConnStr, oSql.ToString(), cVB.nVB_CmdTime);
				//+++++++++++++++++++++++++++++++++++++

                //หา Rate Local
				oSql.Clear();
				oSql.AppendLine($"SELECT FTRteCode, FCRteRate, FTRteType, FTRteIsoCode, FCRteMaxUnit, FTRteTypeChg "); 
				oSql.AppendLine($"FROM TFNMRate WITH(NOLOCK) ");
				oSql.AppendLine($"WHERE FTRteStaLocal = '1' AND FTRteStaUse = '1' ");
				oSql.AppendLine($"AND FTAgnCode = '{tC_AgnCode}' ");
				oDbTblResult = oDB.C_DAToExecuteQuery(tC_tConnStr, oSql.ToString(), cVB.nVB_CmdTime);
				if (oDbTblResult != null && oDbTblResult.Rows.Count > 0)
				{
                    cC_RteLocal = oDbTblResult.Rows[0].Field<decimal>("FCRteRate");
                    tC_IsoCodeLocal = oDbTblResult.Rows[0].Field<string>("FTRteIsoCode");
					tC_IsoNameLocal = cRate.C_GETtIsoName(tC_tConnStr, tC_AgnCode, tC_IsoCodeLocal);
				}
                else
                {
                    //ถ้าไม่มีเอา Rate ตามประเทศ
					oSql.Clear();
					oSql.AppendLine($"SELECT FTRteCode, FCRteRate, FTRteType, FTRteIsoCode, FCRteMaxUnit, FTRteTypeChg ");
					oSql.AppendLine($"FROM TFNMRate WITH(NOLOCK) ");
					oSql.AppendLine($"WHERE FTRteStaUse = '1' ");
					oSql.AppendLine($"    AND FTAgnCode = '{tC_AgnCode}' ");
					oSql.AppendLine($"    AND FTRteIsoCode = '{tC_IsoCodeCty}' ");
					oDbTblResult = oDB.C_DAToExecuteQuery(tC_tConnStr, oSql.ToString(), cVB.nVB_CmdTime);
					if (oDbTblResult != null && oDbTblResult.Rows.Count > 0)
					{
						cC_RteLocal = oDbTblResult.Rows[0].Field<decimal>("FCRteRate");
                    ...
                    {
						//ถ้าไม่มี เอาตาม Company
						oSql.Clear();
						oSql.AppendLine($"SELECT TOP 1 CMP.FTRteCode, RTE.FCRteRate, RTE.FTRteIsoCode ");
						oSql.AppendLine($"FROM TCNMComp CMP WITH(NOLOCK) ");
						oSql.AppendLine($"INNER JOIN TFNMRate RTE WITH(NOLOCK) ON ");
						oSql.AppendLine($"    CMP.FTRteCode = RTE.FTRteCode ");
						oDbTblResult = oDB.C_DAToExecuteQuery(tC_tConnStr, oSql.ToString(), cVB.nVB_CmdTime);
						if (oDbTblResult != null && oDbTblResult.Rows.Count > 0)
						{
							cC_RteLocal = oDbTblResult.Rows[0].Field<decimal>("FCRteRate");
        ...
        public DataSet C_GETxDataPrint(string ptBchCode, string ptDocNo, string ptStaSumPrn, bool pbAlwPrnVoid, string ptConnStr, int pnTimeOut)
        {
            ...
            try
            {
                ...
                oSql.AppendLine("CASE WHEN ISNULL(TXNRed.FCRedPntB4Bill,0) <> 0 THEN ISNULL(TXNRed.FCRedPntB4Bill,0)");
                oSql.AppendLine("WHEN ISNULL(TXNSale.FCTxnPntB4Bill,0) <> 0 THEN ISNULL(TXNSale.FCTxnPntB4Bill,0) ");
                oSql.AppendLine("ELSE ISNULL((SELECT TOP 1 ISNULL(FCTxnPntQty,0) FROM TCNTMemPntActive WHERE FTMemCode=HD.FTCstCode AND FTCgpCode=Cst.FTCgpCode ORDER BY FDTxnPntLast DESC),0) ");
				...
				oSql.AppendLine(", RCV.FCXrcNet, RCV.FTRteCode, RCV.FCXrcRteFac, RCV.FTXrcRteChg, RCV.FCXrcRteChgFac "); //*Net 65-09-09
                ...
                oSql.AppendLine(", FTXshCtrName"); //Net 65-09-10
        ...
        {
            StringBuilder oSql;//Net 65-09-10
			cDatabase oDB;//Net 65-09-10
            ...
            try
            {
                #region Initial
                ////////////////////////////////////////////
                oSql = new StringBuilder();//Net 65-09-10
				oDB = new cDatabase();//Net 65-09-10
				...
				string tRdDisBill = "";
				string tRdPdtBill = "";
				switch (tC_FmtPntSlip)
				{
					case "001": // ADA
					case "002": // MOSHI
						tDisBill = oC_Resource.GetString("tDis");
						tRdDisBill = oC_Resource.GetString("tRdDis");
						tRdPdtBill = oC_Resource.GetString("tRdPdt"); 
						pnStartY += 30;
						break;
					case "003": // SKC
						tDisBill = oC_Resource.GetString("tDisBill");
						tRdDisBill = oC_Resource.GetString("tRdDisBill");
						tRdPdtBill = tRdDisBill;
						pnStartY += 18;
						break;
				}
                //++++++++++++++++++++++++++++++++++
				...
				if (oDbTblHD != null)
                {
                    if (Convert.ToDecimal(oDbTblHD.Rows[0]["FCXshDis"]) != (decimal)0 || Convert.ToDecimal(oDbTblHD.Rows[0]["FCXshChg"]) != (decimal)0 || Convert.ToDecimal(oDbTblHD.Rows[0]["FCXshRnd"]) != (decimal)0)
                    {
                        ...
                        poGraphic.DrawString(oC_Resource.GetString("tSlpSubTotal"), aoC_Font[4], Brushes.Black, 0, pnStartY); //Net 65-09-10
                            ...
                            foreach (DataRow oHDDis in oDbTblHDDis.Rows)
                            {
                                if (string.IsNullOrEmpty(oHDDis.Field<string>("FTXhdRefCode")))
                                {
                                    ...
                                    if (oHDDis.Field<string>("FTXhdDisChgType") == "1" || oHDDis.Field<string>("FTXhdDisChgType") == "2")
                                    {
                                        ...
                                        poGraphic.DrawString(tDisBill + "(" + oHDDis.Field<string>("FTXhdDisChgTxt") + ")", aoC_Font[4], Brushes.Black, 0, pnStartY); //Net 65-09-10
                                ...
                                else
                                {
                                    ...
                                    if (oHDDis.Field<string>("FTXhdDisChgType") == "1")
									{
										//*Net 65-09-10
										oSql.Clear();
										oSql.AppendLine("SELECT TOP 1 FTRdhDocType FROM TPSTSalRD WITH(NOLOCK)");
										oSql.AppendLine("WHERE FTBchCode = '" + poSalePos.ptBchCode + "'");
										oSql.AppendLine("AND FTXshDocNo = '" + poSalePos.ptXihDocNo+ "'");
										oSql.AppendLine("AND FTXrdRefCode = '" + oHDDis.Field<string>("FTXhdRefCode") + "'");
                                        string tRdhDocType = oDB.C_DAToExecuteQuery<string>(tC_tConnStr, oSql.ToString(), cVB.nVB_CmdTime);
										//++++++++++++++
										...
										if (tRdhDocType == "1")
                                        {
                                            ...
                                            poGraphic.DrawString(tRdPdtBill + "(" + oHDDis.Field<string>("FTXhdDisChgTxt") + ")", aoC_Font[4], Brushes.Black, 0, pnStartY);
                                        }
                                        else
                                        {
                        ...
                        if (Convert.ToDecimal(oDbTblHD.Rows[0]["FCXshRnd"]) != 0m)
                        {
                            ...
                            poGraphic.DrawString(oC_Resource.GetString("tSlpRound") +" : " + tAmt, aoC_Font[4], Brushes.Black, 0, pnStartY); //Net 65-09-10
					...
					poGraphic.DrawString(oC_Resource.GetString("tSlpTotal") + "(" + oC_Resource.GetString("tSlpVatIncluded") + ")", aoC_Font[4], Brushes.Black, 0, pnStartY);//Net 65-09-10
                    ...
                    //Net 65-09-10
                    tAmt = oC_Resource.GetString("tSlpVatable") + " : " + oC_SP.SP_SETtDecShwSve(1, Convert.ToDecimal(oDbTblHD.Rows[0]["FCXshVatable"]), nC_DecShow);
                    tAmt += " " + oC_Resource.GetString("tSlpVat") + " : " + oC_SP.SP_SETtDecShwSve(1, Convert.ToDecimal(oDbTblHD.Rows[0]["FCXshVat"]), nC_DecShow);
                    poGraphic.DrawString(tAmt, aoC_Font[4], Brushes.Black, 0, pnStartY);
                }

                if (oDbTblRC != null)
                {
                    if (cRate.C_CHKbDiffIsoRate(tC_IsoCodeLocal, tC_IsoCodeCty)) //Net 65-09-06
                    {
                        C_PRCxPrintIsoRateDiff(ref poGraphic, ref pnStartY, pnWidth, oDbTblHD, oDbTblRC);
                    ...
                    else
                    {
                        foreach (DataRow oRC in oDbTblRC.Rows)
                        {
							...
							tAmt = oC_SP.SP_SETtDecShwSve(1, oRC.Field<Decimal>("FCXrcNet") + oRC.Field<Decimal>("FCXrcChg"), nC_DecShow); //Net 65-09-10
							...
							{
								pnStartY += 18;
								poGraphic.DrawString("Ref No: " + oRC.Field<string>("FTXrcRefNo1"), aoC_Font[6], Brushes.Black, 0, pnStartY);
							}
							//+++++++++++++
						}
                        ...
                        poGraphic.DrawString(oC_Resource.GetString("tSlpChange"), aoC_Font[4], Brushes.Black, 0, pnStartY); //Net 65-09-10
                ...
                if (!string.IsNullOrEmpty(oDbTblHD.Rows[0]["FTCstCode"].ToString()))
                {
                    ...
                    if (oDbTblHDCst != null)
                    {
                        ...
                        //cPntB4Bill = oDbTblHD.Rows[0].Field<decimal>("FCPntB4Bill"); //Net 65-09-10
                        ...
                        if (!string.IsNullOrEmpty(oDbTblHDCst.Rows[0]["FTXshDocNo"].ToString()))
                        {
                            ...
                            poGraphic.DrawString(oC_Resource.GetString("tSlpMemCode") + " : " + oDbTblHD.Rows[0]["FTCstCode"].ToString(), aoC_Font[1], Brushes.Black, 0, pnStartY);

                            pnStartY += 18;
							//Net 65-09-10
							//poGraphic.DrawString(oDbTblHDCst.Rows[0]["FTXshCstName"].ToString(), aoC_Font[1], Brushes.Black, 10, pnStartY);
							string tName = "";
							//* ปรับ Slip ให้สามารถแสดง ชื่อและนามสกุลของลูกค้าสมาชิก 
							switch (tC_FmtPntSlip)
							{
								case "001": // ADA
								case "002": // MOSHI
									if (!string.IsNullOrEmpty(oDbTblHDCst.Rows[0]["FTXshCstName"].ToString()))
									{
										string[] aName = oDbTblHDCst.Rows[0]["FTXshCstName"].ToString().Split(' ');
										...
										switch (aName.Length)
										{
											...
											case 1:
											case 2:
												tName = aName[0].ToString();
												break;
											default:
												tName = aName[0].ToString() + " " + aName[1].ToString();
												break;
										}
									}
									break;
								case "003": // SKC
									tName = oDbTblHDCst.Rows[0]["FTXshCstName"].ToString();
									break;
							}
							poGraphic.DrawString(tName, aoC_Font[1], Brushes.Black, 10, pnStartY);
							//+++++++++++++
							...
							poGraphic.DrawString(oC_Resource.GetString("tSlpMemCard") + " : " + tCardNo, aoC_Font[1], Brushes.Black, 0, pnStartY); //Net 65-09-10
                            ...
                            if (!string.IsNullOrEmpty(oDbTblHD.Rows[0].Field<string>("FDCstCrdExpire")))
                            {
								...
								poGraphic.DrawString(oC_Resource.GetString("tSlpMemExpire") + " : " + DateTime.ParseExact(oDbTblHD.Rows[0].Field<string>("FDCstCrdExpire"), "yyyy-MM-dd", null).ToString("dd-MM-yyyy"), aoC_Font[1], Brushes.Black, 0, pnStartY);
                            }
                            else
                            {
								//Net 65-09-10
								//poGraphic.DrawString("Expired : -", aoC_Font[1], Brushes.Black, 0, pnStartY);
                                poGraphic.DrawString(oC_Resource.GetString("tSlpMemExpire") + " : -", aoC_Font[1], Brushes.Black, 0, pnStartY);
                            }

							//*Net 65-09-10
							pnStartY += 18;
                            if (!string.IsNullOrEmpty(oDbTblHDCst.Rows[0]["FTXshCtrName"].ToString()))
                            {
								poGraphic.DrawString(oC_Resource.GetString("tSlpCtrName") + " : " + oDbTblHDCst.Rows[0]["FTXshCtrName"].ToString(), aoC_Font[1], Brushes.Black, 0, pnStartY);
                            }
                            else
                            {
								poGraphic.DrawString(oC_Resource.GetString("tSlpCtrName") + " : -", aoC_Font[1], Brushes.Black, 0, pnStartY);
                            }
                            //+++++++++++++
							...
							//*Net 65-09-10
							poGraphic.DrawString(oC_Resource.GetString("tSlpMemLastPt"), aoC_Font[1], Brushes.Black, 0, pnStartY);      
							poGraphic.DrawString(oC_Resource.GetString("tSlpMemRegPt"), aoC_Font[1], Brushes.Black, 70, pnStartY);      
							poGraphic.DrawString(oC_Resource.GetString("tSlpMemPmtPt"), aoC_Font[1], Brushes.Black, 140, pnStartY);     
							poGraphic.DrawString(oC_Resource.GetString("tSlpMemTotalPt"), aoC_Font[1], Brushes.Black, 210, pnStartY);   
				...
				//*Net 65-09-10 Starndard ตัดนามสกุล
				string tUserName = "";
				if (!string.IsNullOrEmpty(oDbTblHD.Rows[0]["FTUsrName"].ToString()))
				{
					string[] aName = oDbTblHD.Rows[0]["FTUsrName"].ToString().Split(' ');
					...
					switch (aName.Length)
					{
						...
						case 1:
						case 2:
							tUserName = aName[0].ToString();
							break;
						default:
							tUserName = aName[0].ToString() + " " + aName[1].ToString();
							break;
					}
				}
				//+++++++++++++
                ...
                poGraphic.DrawString(oC_Resource.GetString("tSlpUsrName") + " : " + tUserName, aoC_Font[1], Brushes.Black, 0, pnStartY);//*Net 65-09-10
                ...
                if (!string.IsNullOrEmpty(oDbTblHD.Rows[0]["FTXshRmk"].ToString()))
                {
					...
					{
						pnStartY += 18;
						if (poGraphic.MeasureString(tStr, aoC_Font[4]).Width > pnWidth)
						{
							string[] aMsg = tStr.Split(' ');
							tMsg = "";
							for (int nIndex = 0; nIndex < aMsg.Length; nIndex++)
							{
								...
								{
									if (poGraphic.MeasureString(tMsg + " " + aMsg[nIndex], aoC_Font[4]).Width > pnWidth)
									{
										int nLen = 1;
										tMsg = tMsg + " " + aMsg[nIndex];
										tPrintText = tMsg.Substring(0, tMsg.Length - nLen);
										while (poGraphic.MeasureString(tPrintText, aoC_Font[4]).Width > pnWidth)
										{
											nLen++;
											tPrintText = tMsg.Substring(0, tMsg.Length - nLen);
											if (poGraphic.MeasureString(tMsg.Substring(tMsg.Length - nLen), aoC_Font[4]).Width > pnWidth)
											{
												poGraphic.DrawString(tPrintText, aoC_Font[4], Brushes.Black, new RectangleF(0, pnStartY, pnWidth, 18));
												tMsg = tMsg.Substring(tMsg.Length - nLen);
												pnStartY += 18;
												nLen = 1;
												tPrintText = tMsg.Substring(0, tMsg.Length - nLen);
											}
							...
							if (tMsg != "")
							{
								...
								if (poGraphic.MeasureString(tMsg, aoC_Font[4]).Width > pnWidth)
								{
									int nLen = 1;
									tPrintText = tMsg.Substring(0, tMsg.Length - nLen);
									while (poGraphic.MeasureString(tPrintText, aoC_Font[4]).Width > pnWidth)
									...
									tMsg = tMsg.Substring(tMsg.Length - nLen);
									pnStartY += 18;
									poGraphic.DrawString(tMsg, aoC_Font[4], Brushes.Black, new RectangleF(0, pnStartY, pnWidth, 18));
								}
								else
								{
									poGraphic.DrawString(tMsg, aoC_Font[4], Brushes.Black, new RectangleF(0, pnStartY, pnWidth, 18));
						...
						{
							poGraphic.DrawString(tStr, aoC_Font[4], Brushes.Black, new RectangleF(0, pnStartY, pnWidth, 18));
						}
						//++++++++++++++
					}
				}
            ...
            catch (Exception oEx)
            {
                ...
                new cLog().C_WRTxLog("cEJ", "C_PRNxSlip : " + oEx.Message);
            }
            finally
            {
                //oC_SP.SP_CLExMemory();
        ...
        public bool C_GENbSlip(cmlRcvSalePos poSalePos, cmlShopDB poShopDB, cmlTCNMPos poPos, ref string ptErrMsg)
        {
            ...
            try
            {
				...
				C_GETxConfigBranch(poSalePos.ptBchCode); //Net 65-09-06
		...
		public void C_PRCxPrintIsoRateDiff(ref Graphics poGraphic, ref int pnStartY, int pnWidth, DataTable poTblHD, DataTable poTblRC)
		{
			StringFormat oFormatFar;
			StringFormat oFormatCenter;
			decimal cAmt, cAmtTo;
			string tAmt, tAmtTo;

			decimal cChange;
			decimal cChgAmt, cChgAmtTo;
			string tChgAmt, tChgAmtTo;

			string tText = "";
			try
			{
				if (cRate.C_CHKbDiffIsoRate(tC_IsoCodeLocal, tC_IsoCodeCty) == false) return;

				oFormatFar = new StringFormat() { Alignment = StringAlignment.Far };
				oFormatCenter = new StringFormat() { Alignment = StringAlignment.Center };

				//ยอดสุทธิ
				cAmt = Convert.ToDecimal(poTblHD.Rows[0]["FCXshGrand"]);
				cAmtTo = cRate.C_PRCcConvertIsoCode(tC_tConnStr, tC_AgnCode, tC_IsoCodeLocal, tC_IsoCodeCty, cAmt);
				tAmt = new cSP().SP_SETtDecShwSve(1, cAmt, nC_DecShow);
				tAmtTo = new cSP().SP_SETtDecShwSve(1, cAmtTo, nC_DecShow);

				pnStartY += 18;
				poGraphic.DrawString(oC_Resource.GetString("tSlpTotal") + "(" + tC_IsoNameCty + ")", aoC_Font[4], Brushes.Black, 0, pnStartY);
				poGraphic.DrawString(tAmtTo, aoC_Font[5], Brushes.Black, new RectangleF(140, pnStartY, pnWidth - 150, 18), oFormatFar);

				//ยอดตามประเภทชำระเงิน
				cChange = 0;
				foreach (DataRow oRC in poTblRC.Rows)
				{
					//ยอดชำระ
					//cAmt = Convert.ToDecimal(oRC.Field<Decimal>("FCXrcUsrPayAmt"));
					cAmt = Convert.ToDecimal(oRC.Field<Decimal>("FCXrcNet")) + oRC.Field<Decimal>("FCXrcChg"); //Net 65-09-04
					cAmtTo = cRate.C_PRCcConvertIsoCode(tC_tConnStr, tC_AgnCode, tC_IsoCodeLocal, tC_IsoCodeCty, cAmt);
					tAmt = new cSP().SP_SETtDecShwSve(1, cAmt, nC_DecShow);
					tAmtTo = new cSP().SP_SETtDecShwSve(1, cAmtTo, nC_DecShow);
					...
					pnStartY += 18;
					if (cRate.C_GETtIsoCode(tC_tConnStr, tC_AgnCode, oRC.Field<string>("FTRteCode")) == tC_IsoCodeCty)
					{
						tText = $"{oRC.Field<string>("FTRcvName")} ({tC_IsoNameCty})";
						poGraphic.DrawString(tText, aoC_Font[6], Brushes.Black, 0, pnStartY);
						poGraphic.DrawString(tAmtTo, aoC_Font[7], Brushes.Black, new RectangleF(140, pnStartY, pnWidth - 150, 18), oFormatFar);
					}
					else
					{
						tText = $"{oRC.Field<string>("FTRcvName")} {tC_IsoNameLocal}({tAmt} @{cC_RteCty} {tC_IsoNameCty}/{tC_IsoNameLocal})";
						poGraphic.DrawString(tText, aoC_Font[6], Brushes.Black, 0, pnStartY);
						poGraphic.DrawString(tAmtTo, aoC_Font[7], Brushes.Black, new RectangleF(140, pnStartY, pnWidth - 150, 18), oFormatFar);
					}
				...
				//ยอดเงินทอน
				cChgAmt = cChange;
				cChgAmtTo = cRate.C_PRCcConvertIsoCode(tC_tConnStr, tC_AgnCode, tC_IsoCodeLocal, tC_IsoCodeCty, cChgAmt);
				tChgAmt = new cSP().SP_SETtDecShwSve(1, cChgAmt, nC_DecShow);
				tChgAmtTo = new cSP().SP_SETtDecShwSve(1, cChgAmtTo, nC_DecShow);

				pnStartY += 18;
				poGraphic.DrawString(oC_Resource.GetString("tSlpChange") + $" ({tC_IsoNameCty})", aoC_Font[4], Brushes.Black, 0, pnStartY);
				poGraphic.DrawString(tChgAmtTo, aoC_Font[5], Brushes.Black, new RectangleF(140, pnStartY, pnWidth - 150, 18), oFormatFar);

			}
			catch (Exception oEx)
			{
				new cLog().C_WRTxLog("cEJ", "C_PRCxPrintIsoRateDiff : " + oEx.Message);
			}
