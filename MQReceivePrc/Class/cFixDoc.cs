    ...
    public class cFixDoc
    {
        ...
        public List<cmlListSalDoc> C_GEToFixDoc(string ptConnStr, List<cmlListSalDoc> poPosDoc, int pnTimeOut)
        {
            ...
            try
            {
                ...
                //*Net 63-09-30 update จำนวนบิล Pos ที่ TPSTShiftHD
                oSql.Clear();
                oSql.AppendLine("IF EXISTS(SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'TPSTShiftHD' AND COLUMN_NAME = 'FNShdQtyBill') BEGIN");
                oSql.AppendLine("   UPDATE TPSTShiftHD");
                oSql.AppendLine($"  SET FNShdQtyBill={((aoPosDoc == null) ? 0 : aoPosDoc.Count)}");
                oSql.AppendLine($"  WHERE FTBchCode='{aoPosDoc[0].ptFTBchCode}' AND FTPosCode='{aoPosDoc[0].ptFTPosCode}' AND FTShfCode='{aoPosDoc[0].ptFTShfCode}'");
                oSql.AppendLine("END");
                oDB.C_DATbExecuteNonQuery(ptConnStr, oSql.ToString(), pnTimeOut, out nRowEff);
                //++++++++++++++++++++++++++++++++++++++
